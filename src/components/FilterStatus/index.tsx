...
export default function FilterStatus({ ...props }: IFilterStatus) {
  return (
    <TouchableOpacity
      ...
      style={{
        backgroundColor: props.status === props.statusSelected ? "#3394F1" : "#111C2A",
      }}
    >
      <Text
        className="font-interSemiBold text-[16px]"
        style={{ color: props.status === props.statusSelected ? "#FFFFFF" : "#C0C7D4" }}
      >
        {props.status}
      </Text>
      <View
        ...
        style={{
          backgroundColor: props.status === props.statusSelected ? "#111C2A" : "#31353C",
        }}
      >
        <Text
