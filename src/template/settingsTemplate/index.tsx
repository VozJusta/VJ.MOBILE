...
function getHelpIconPalette(itemId: string) {
  switch (itemId) {
    case "ai-overview":
      return {
        bg: "rgba(17, 85, 170, 0.22)",
        border: "rgba(67, 141, 231, 0.32)",
        color: "#44A4FF",
      };
    case "plans-billing":
      return {
        bg: "rgba(180, 132, 28, 0.2)",
        border: "rgba(233, 172, 41, 0.36)",
        color: "#F4B72D",
      };
    case "process-questions":
      return {
        bg: "rgba(11, 140, 136, 0.2)",
        border: "rgba(47, 185, 168, 0.34)",
        color: "#2DDFC4",
      };
    case "security-data":
      return {
        bg: "rgba(170, 33, 86, 0.2)",
        border: "rgba(243, 84, 130, 0.34)",
        color: "#FF4F8D",
      };
    case "getting-started":
      return {
        bg: "rgba(96, 62, 196, 0.22)",
        border: "rgba(150, 110, 255, 0.36)",
        color: "#A78BFA",
      };
    default:
      return {
      ...
      >
        <View className="flex-1 flex-row items-center gap-[12px] pr-[14px]">
          <MaterialIcons
            name={item.icon}
            size={21}
            color={isDanger ? "#F87171" : "#E2E8F0"}
          />

          <View className="flex-1">
            <Text
              className={`text-[18px] font-interRegular ${isDanger ? "text-[#F87171]" : "text-[#F1F5F9]"}`}
            >
              {item.label}
            </Text>
        ...
        <View className="flex-1 flex-row items-start gap-[11px] pr-[14px]">
          {showIconBubble && (
            ...
            >
              <MaterialIcons
                name={item.icon}
                size={isPrivacy ? 20 : 20}
                color={
                  isDanger
                    ? "#F87171"
                    : isHelp
                      ? helpIconPalette?.color
                      : "#60A5FA"
                }
              />
            </View>
          ...
          <View className="flex-1">
            <Text
              className={`${isPrivacy ? "text-[16px]" : isHelp ? "text-[17px]" : "text-[18px]"} font-interMedium ${isDanger ? "text-[#F87171]" : "text-[#F1F5F9]"}`}
            >
              {item.label}
            </Text>
            ...
            {!!item.description && (
              <Text
                className={`${isPrivacy ? "text-[13px] leading-[21px]" : isHelp ? "text-[13px]" : "text-[14px]"} text-[#94A3B8] font-interRegular mt-[5px]`}
              >
                {item.description}
              </Text>
            )}
...
function SettingsSectionBlock({
  ...
  return (
    <View className={isPrivacy ? "  gap-[13px]" : "gap-[10px]"}>
      <Text
        className={`uppercase tracking-[2.3px] text-[11px] font-interBold ${isPrivacy ? "text-[#7E96B9]" : isHelp ? "text-[#1B99EB]" : "text-[#94A3B8]"}`}
      >
        {section.title}
      </Text>
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        ...
        {!!description && (
          <Text
            className={`${isPrivacy ? "text-[#AFC3DF] text-[16px] leading-[30px] mb-[28px]" : "text-[#B8C5DA] text-[20px] leading-[28px] mb-[20px]"} font-interRegular`}
          >
            {description}
          </Text>
        )}
        ...
        {!!supportCard && !isHelp && (
          <View
            className={
              isHelp
                ? "mb-2 mt-[20px]"
                : "p-[1px] rounded-[18px] bg-[#135BEC]/70 mb-2 mt-[8px]"
            }
          >
            {isHelp ? (
              <Pressable
              ...
              >
                ...
                >
                  <View className="flex-row items-center justify-center gap-[10px]">
                    <MaterialIcons
                      name={supportCard.icon ?? "support-agent"}
                      size={25}
                      color="#FFFFFF"
                    />
              ...
              <View className="rounded-[18px] bg-[rgba(2,6,23,0.88)] px-[14px] py-[16px] flex-row items-center justify-between">
                <View className="flex-row items-center gap-[12px] flex-1 pr-[10px]">
                  <View
                    className={`items-center justify-center ${isHelp ? "w-[36px] h-[36px]" : "w-[40px] h-[40px] rounded-full bg-[#135BEC]/20"}`}
                  >
                    <MaterialIcons
                      name={supportCard.icon ?? "support-agent"}
                      size={isHelp ? 28 : 22}
                      color={isHelp ? "#FFFFFF" : "#60A5FA"}
                    />
                  </View>

                  <View className="flex-1">
                    <Text
                      className={`${isHelp ? "text-white text-[20px]" : "text-white text-[18px]"} font-interBold`}
                    >
                      {supportCard.title}
                    </Text>
                    {!!supportCard.description && (
                      <Text
                        className={`${isHelp ? "text-white/75 text-[12px]" : "text-[#94A3B8] text-[13px]"} font-interRegular`}
                      >
                        {supportCard.description}
                      </Text>
                    )}
        ...
        {!!dangerCard && (
          ...
          >
            <View className="flex-row items-center gap-[12px]">
              <View
                className={`${isPrivacy ? "w-[40px] h-[40px]" : "w-[34px] h-[34px]"} rounded-full bg-red-500/20 items-center justify-center`}
              >
                <MaterialIcons
                  name="warning"
                  size={isPrivacy ? 18 : 16}
                  color="#FF5E69"
                />
              </View>
              <Text
                className={`${isPrivacy ? "text-[18px]" : "text-[18px]"} text-[#FF5E69] font-interBold`}
              >
                {dangerCard.title}
              </Text>
            </View>
            ...
            <Text
              className={`${isPrivacy ? "text-[14px] text-[#FECACA]/60 leading-[24px]  mt-[15px] mb-[32px]" : "text-[14px] leading-[22px] mt-[12px]"} text-[#9CA3AF] font-interRegular`}
            >
            ...
            <ButtonUI
              ...
              size={
                isPrivacy ? "h-[56px] w-full rounded-[16px]" : "h-[48px] w-full"
              }
              iconLeft={false}
            ...
            >
              <View className="w-full h-full items-center justify-center">
                <Text
                  className={`${isPrivacy ? "text-[16px] tracking-[0.8px]" : "text-[16px] tracking-[0.7px]"} text-white font-interBold uppercase`}
                >
